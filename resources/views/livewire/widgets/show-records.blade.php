    ...
    @if (isset($records) && $records->count() > 0)
        @foreach ($records as $record)
            <div wire:loading.remove class="flex flex-row justify-evenly">
                ...
                @if ($record->type == "MX")
                    <div class="relative z-0 w-1/5 mb-6 group">
                        <input disabled type="number" name="priority" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" value="{{$record->prio}}"/>
